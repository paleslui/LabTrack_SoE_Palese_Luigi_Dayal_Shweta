...
| Anomaly detection on status sequences | Could flag unusual lifecycle paths | A 5-state machine has too few transitions to produce meaningful anomalies; the audit log is sufficient for spot-checking unusual paths |
...
3. **Complexity cost outweighs benefit.** The course rubric rewards *justification*, not ML for its own sake. Adding a half-baked model (e.g., a random forest on 5 features) would introduce a new dependency (scikit-learn), training infrastructure, and model versioning overhead, all for a system whose value comes from reliability, not prediction.

4. **Team feasibility.** The team has mixed programming backgrounds (constraint noted in Stage 1). Implementing, testing, and maintaining an ML component would redirect effort away from the core deliverables that directly serve the lab users.
